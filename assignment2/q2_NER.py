  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
       ...
       with tf.name_scope("Embedding"):
         embedings = tf.Variable(tf.random_uniform([len(self.wv),self.config.embed_size],-1,1))
#         embd = tf.nn.embedding_lookup(self.wv,self.input_placeholder)
         embd = tf.nn.embedding_lookup(embedings,self.input_placeholder)
         window = tf.reshape(embd,(-1,self.config.window_size*self.config.embed_size))
#       raise NotImplementedError
       ### END YOUR CODE
       return window
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
        W = tf.Variable(xavier_initializer((self.config.window_size*self.config.embed_size,self.config.hidden_size)),
                        dtype = tf.float32,name = "W")
        b1 = tf.zeros((self.config.hidden_size,),name = "b1")
        h = tf.nn.tanh(tf.matmul(window,W) + b1, name = "h")
    
    h_droped = tf.nn.dropout(h,self.dropout_placeholder)
    
    with tf.variable_scope("Softmax"):
#        U = xavier_initializer((self.config.hidden_size,self.config.label_size))
        U = tf.Variable(xavier_initializer((self.config.hidden_size,self.config.label_size)),
                        dtype = tf.float32,name = "U")
        b2 = tf.zeros((self.config.label_size,),name = "b2")
        output = tf.nn.softmax(tf.matmul(h_droped,U) + b2,name = "output")
    
    output = tf.nn.dropout(output,self.dropout_placeholder)
    
    with tf.name_scope("Regularization"):
        reg_W = tf.reduce_sum(tf.pow(W,2))
        reg_U = tf.reduce_sum(tf.pow(U,2))
        loss_reg = self.config.l2*(reg_W + reg_U)
        tf.add_to_collection("reg_tem",loss_reg)  
...
#    raise NotImplementedError
  ...
  def add_loss_op(self, y):
    ...
    with tf.name_scope("total_loss"):
        ...
        loss_reg = tf.get_collection("reg_tem")[0]
        loss += loss_reg
...
#    raise NotImplementedError
  ...
  def add_training_op(self, loss):
    ...
    with tf.name_scope("Train"):
      model = tf.train.GradientDescentOptimizer(learning_rate=self.config.lr)
