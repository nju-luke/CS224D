...
#                    tf.random_uniform([len(self.vocab),self.config.embed_size],-1,1))
        ...
        inputs = [tf.squeeze(x,squeeze_dims=[1]) for x in tf.split(1,self.config.num_steps,embed)]
#        print inputs
...
#      U = tf.Variable(tf.random_uniform([self.config.hidden_size,len(self.vocab)],-1,1))
#      b_2 = tf.Variable(tf.zeros([len(self.vocab),]))
      U = tf.get_variable("U",[self.config.hidden_size,len(self.vocab)])
      b_2 = tf.get_variable("b_2",[len(self.vocab),])
      outputs = [tf.matmul(outp_i,U) + b_2 for outp_i in rnn_outputs]
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    ### YOUR CODE HERE
    targets = [tf.to_int32(
              tf.reshape(tf.concat(0,self.labels_placeholder)
              ,[-1]))]
    logits = [output]
    weight = [tf.ones([])]

    loss = sequence_loss(logits,targets,weight)
    ### END YOUR CODE
    return loss
...
#    H = tf.Variable(tf.random_uniform([self.config.hidden_size,self.config.hidden_size],-1,1))
#    I = tf.Variable(tf.random_uniform([self.config.embed_size,self.config.hidden_size],-1,1))
#    b_1 = tf.Variable(tf.zeros([self.config.hidden_size,]))
    with tf.variable_scope("RNN"):
      ...
      b_1 = tf.get_variable("b",[self.config.hidden_size,])
    
    rnn_outputs = []
    h = self.initial_state
    for inp_i in inputs:
      inp_i = tf.nn.dropout(inp_i,self.config.dropout)      
      h_pre = tf.nn.dropout(h,self.config.dropout)
      
      h = tf.nn.sigmoid(tf.matmul(h_pre,H) + tf.matmul(inp_i,I) + b_1)
...
#            model.dropout_placeholder: dp}
    y_pred,state = session.run([model.predictions[-1],model.final_state],feed)
