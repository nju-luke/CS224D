...
### YOUR CODE HERE
REGULARIZATION = np.linspace(0.2,0.6,20)
### END YOUR CODE
...
for regularization in REGULARIZATION:
    ...
    weights = sgd(lambda weights: softmax_wrapper(trainFeatures, trainLabels,
        weights, regularization), weights, 3.0, 1000, PRINT_EVERY=200)
...
for result in results:
    ...
    dev_acc.append(result['dev'])

BEST_REGULARIZATION = REGULARIZATION[np.argmax(dev_acc)]
BEST_WEIGHTS = sgd(lambda weights: softmax_wrapper(trainFeatures, trainLabels,
        weights, BEST_REGULARIZATION), weights, 3.0, 10000, PRINT_EVERY=1000)
### END YOUR CODE
