...
class Config(object):
    """Holds model hyperparams and data information.
       Model objects are raise NotImplementedErrored a Config() object at instantiation.
    """
    embed_size = 35
    label_size = 2
    early_stopping = 2
...
class RNN_Model():
    ...
    def load_data(self):
        ...
        self.train_data, self.dev_data, self.test_data = tr.simplified_data(100, 20, 40)#(700, 100, 200)#

        # build vocab from training data
        self.vocab = Vocab()
        train_sents = [t.get_words() for t in self.train_data]
        self.vocab.construct(list(itertools.chain.from_iterable(train_sents)))
        self.vocab_size = len(self.vocab.word_freq)
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ### YOUR CODE HERE
            self.embedding = tf.get_variable("embedding",[self.vocab_size, self.config.embed_size])
            self.W1 = tf.get_variable("W1",[2*self.config.embed_size, self.config.embed_size],initializer=xavier_initializer)
            self.b1 = tf.get_variable("b1",[1, self.config.embed_size],initializer=tf.zeros)
#            self.W1 = tf.Variable(xavier_initializer((self.vocab_size, self.config.embed_size)),dtype=tf.float32,name="W1")
#            self.b1 = tf.zeros([1, self.config.embed_size],name="b1")
            ### END YOUR CODE
        with tf.variable_scope('Projection'):
            ### YOUR CODE HERE
            self.U = tf.get_variable("U",[self.config.embed_size, self.config.label_size],initializer=xavier_initializer)
            self.bs = tf.get_variable("bs",[1, self.config.label_size],initializer=tf.zeros)
#            self.U = tf.Variable(xavier_initializer((self.config.embed_size, self.config.label_size)),dtype=tf.float32,name="U")            
#            self.bs = tf.zeros([1, self.config.label_size],name="bs")
            ### END YOUR CODE
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            embed = tf.nn.embedding_lookup(self.embedding,self.vocab.encode(node.word))
            curr_node_tensor = tf.reshape(embed,[1,-1])
#            raise NotImplementedError
            ...
            ### YOUR CODE HERE
            concat_tensor = tf.concat(1,[node_tensors[node.left],node_tensors[node.right]],name="concat_tensor")
            node_tensor = tf.matmul(concat_tensor,self.W1) + self.b1
            curr_node_tensor = tf.maximum(node_tensor,0,name="Relu")
            ### END YOUR CODE
        node_tensors[node] = tf.nn.dropout(curr_node_tensor,self.config.dropout)
        return node_tensors
...
#        proj_z = tf.matmul(node_tensors,self.U) + self.bs
#        logits = tf.nn.softmax(proj_z,name="Projection")
    ...
    def loss(self, logits, labels):
        ...
        loss = None
        # YOUR CODE HERE
        loss_ce = tf.nn.sparse_softmax_cross_entropy_with_logits(logits,labels,"CE_loss")
        l2_loss = tf.nn.l2_loss(self.W1) + tf.nn.l2_loss(self.U)
        loss = tf.reduce_mean(loss_ce) + self.config.l2*l2_loss
        # END YOUR CODE
        return loss
    ...
    def training(self, loss):
        ...
        The Op returned by this function is what must be raise NotImplementedErrored to the
...
#    plt.show()
    for ke in stats:
        plt.plot(stats['%s'%ke],hold=False)
        plt.title('%s'%ke)
        plt.xlabel('Iteration')
        plt.savefig("%s.png"%ke)
        plt.show()
